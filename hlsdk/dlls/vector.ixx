...
export struct Vector2D
{
	...
	static consteval Vector2D Right(void) noexcept { return Vector2D(0, -1); }
	static consteval Vector2D Left(void) noexcept { return Vector2D(0, 1); }
	static consteval Vector2D Front(void) noexcept { return Vector2D(1, 0); }
	static consteval Vector2D Back(void) noexcept { return Vector2D(-1, 0); }

	constexpr Vector2D operator+(const Vector2D &v) const noexcept { return Vector2D(x + v.x, y + v.y); }
	constexpr Vector2D operator-(const Vector2D &v) const noexcept { return Vector2D(x - v.x, y - v.y); }
	constexpr Vector2D operator*(double fl) const noexcept { return Vector2D(x * fl, y * fl); }
	constexpr Vector2D operator/(double fl) const noexcept { fl = 1 / fl; return Vector2D(x * fl, y * fl); }
...
{                                                               //              which is a vec_t[3]
	...
	// Methods as Vector2D
	constexpr	Vector2D Make2D(void) const noexcept { return Vector2D(x, y); }
...
export constexpr Vector CrossProduct(const Vector &a, const Vector &b)	noexcept { return Vector(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x); }
...
export struct Angles	// same data-layout as engine's vec3_t
{
	...
	constexpr Angles	&operator-=(const Angles &a)	noexcept	{ pitch -= a.pitch; yaw -= a.yaw; roll -= a.roll; return *this; }
	constexpr Angles	&operator*=(double fl)			noexcept	{ pitch *= fl; yaw *= fl; roll *= fl; return *this; }
	constexpr Angles	&operator/=(double fl)			noexcept	{ pitch /= fl; yaw /= fl; roll /= fl; return *this; }
